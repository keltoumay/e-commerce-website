...
PostmanFs.prototype._resolve = function (relOrAbsPath, insecureFileRead, whiteList = []) {
  ...
  if (!insecureFileRead && // insecureFile read disabled
    ...
    !whiteList.includes(resolvedPath)) { // File not in whitelist
      // Exit
      return;
  }
...
PostmanFs.prototype.resolvePath = function (relOrAbsPath, whiteList, callback) {
  ...
  let resolvedPath = this._resolve(relOrAbsPath, this.insecureFileRead, this.fileWhitelist.concat(whiteList));
...
PostmanFs.prototype.resolvePathSync = function (relOrAbsPath, whiteList) {
  ...
  const resolvedPath = this._resolve(relOrAbsPath, this.insecureFileRead, this.fileWhitelist.concat(whiteList));
